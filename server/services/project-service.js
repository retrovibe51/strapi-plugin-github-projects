...
module.exports = ({strapi}) => ({
    ...
    createAll: async (repos, userId) => {
        const createPromises = repos.map(
            async (repo) => await strapi.plugin("github-projects").service("projectService").create(repo, userId)  // calling the create service function which is defined above in this file itself
        );
        return Promise.all(createPromises);
    },
    deleteAll: async (projectIds) => {
        const deletePromises = projectIds.map(
            async (id) => await strapi.plugin("github-projects").service("projectService").delete(id)
        );
